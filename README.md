...
<tr>
    <td>구대윤</td>
    <td>팀원들의 노력으로 모든 기능을 무사히 끝마칠 수 있었습니다!. 해당 프로젝트가 좀 더 발전 되어서 좋은 포트폴리오로 사용될 수 있도록 노력하겠습니다!</td>
  </tr>
  <tr>
    <td>오민성</td>
    <td>이번 백엔드 프로젝트를 수행하면서 msa에 대해서는 조금 더 깊이 알면 좋을 것 같습니다. 개인적으로 부족하다고 생각하는 첫 번째로는 중복코드가 너무 많아져서 config-server에 중복코드를 동시에 보관할 수 있는 구조에 대해 적용할 수 있을 지에 대해 좀 알아보고 싶습니다. 두 번째로는 msa 구조라는 게 사실 배포까지 생각을 해야하는 것 같은데 그것을 예상해서 docker를 이용해 배포를 해보려했지만 실패하였습니다. 추후에 devops에서 배우게 될 docker에서 함께 배포까지 완성을 한다면 msa에 대해 완벽하게 알게 됐다고 말할 수 있을 것 같습니다.
JPA에 대해 개념과 함께 사용방법까지 알게됐으니 개인공부를 통해 QueryDSL까지 완성해낸다면 JPA도 완벽하게 이해했다고 할 수 있을 것 같습니다.
마지막으로 채팅구현을 하면서 단순하게 WebSocket만을 사용해 구현을 하였는데 이것을 채팅구현을 해냈다고 말할 수 없을 것 같습니다. 프론트엔드 프로젝트를 진행하면서 STOMP 프로토콜과 메시지 브로커를 사용하여 조금 더 완벽한 채팅 구현을 해낼 수 있도록 노력하겠습니다.
매번 그렇듯 처음 해보는 기술은 스스로 구글링, GPT를 통해 구현을 하기에는 옳은 정보를 가려내는데 너무 오랜 시간이 걸리는 것 같습니다..
조원들의 도움이 있어 처음 접해보는 채팅 기능 구현이 오래걸렸음에도 다른 많은 기능들을 구현해냈다고 생각합니다. 
팀원들이 많이 도와주셔서 함께 백엔드 프로젝트 무사히 마쳤습니다 ! 
다들 고생 많으셨고 프론트까지 파이팅 합시다 !</td>
  </tr>
  <tr>
    <td>박상현</td>
    <td>백엔드 프로젝트를 하면서 수업을 들을 때 배웠던 부분을 적용해 볼 수 있는 좋은 기회였다. 실제로 해보니 생각보다 더 어려웠고 팀원들과 어느정도 코드도 맞춰야하니 더 복잡했던 것 같다. 그래도 팀원들이 차근차근 알려주고 도와줘서 포기하지 않고 끝까지 프로젝트를 마무리 할 수 있었던 것 같다. 생각보다 많은 시간이 소요 되는 것을 알았고 몰랐던 부분에 대해서 다시 한번 찾아보고 공부 할 수 있었던 기회가 되었고, 아직 부족하다고 느낀 부분들은 더 꼼꼼하게 공부해야겠다.</td>
  </tr>
  <tr>
    <td>임서연</td>
    <td>처음 진행해보는 백엔드 프로젝트였던 만큼, 항상 스스로가 알맞게 로직을 짜고 있는건지 몰라 긴장의 연속이었던것 같습니다.  제 생각에 확신이 없어 간단한 코드 한줄 작성하는 것에도 습관처럼 강의자료를 찾거나 구글링을 했고, 주말마다 경험이 있는 팀원과 개발자 지인들에게 자문을 구하며 진행하였습니다. 당시에는 시간 대비 너무 비효율적으로 일을 하고 있는 것 같아 자괴감이 들었지만, 프로젝트를 마무리한 지금은 그 과정들이 저에게 있어 백엔드 서비스 작성과 데이터 분석, 알고리줌 설계에 대한 감각을 크게 키워주었다 생각합니다. 그리고 무엇보다 팀원들이 각자의 역할을 충실히 수행하였고, 적극적으로 소통하고 피드백을 주고받으며 여러 이슈들을 함께 해결하였기에 프로젝트를 무사히 마칠 수 있었습니다.</td>
  </tr>
  <tr>
    <td>전아영</td>
    <td>프로젝트의 기획부터 개발까지 진행하면서 전체적인 흐름을 배울 수 있었던 점이 좋았습니다.
그리고 CQRS 패턴을 직접 적용해봄으로써 단순히 확장성, 보안적인 측면의 장점 뿐만이 아닌
데이터의 일관성, 아키텍쳐의 복잡성 등 장단점을 확실하게 느낄 수 있었습니다.
프로젝트 기간이 길지 않아 걱정이 많았는데 열정적인 팀원들과 함께하면서 빠르게 잘 마무리할 수 있었던 것 같습니다.
이 과정에서 팀워크의 중요성을 다시 한 번 느꼈고, 함께 노력하며 성장했던 팀원들에게 감사드립니다.</td>
  </tr>
